...
angular.module('starter.controllers', [])
  .controller('editOrderCtrl', function($scope, $state,$compile, HttpService, $ionicSlideBoxDelegate, $ionicTabsDelegate,$stateParams,$ionicPlatform,customerHost,smModal,Util,TakePhoto,$ionicHistory) {
    // 返回
    ...
    $scope.baoHeight = { 'height': window.screen.height-100 + 'px' };
    $scope.repairs = [];
    $scope.repairsItem = [];
    $scope.repairsMaterial = [];
    $scope.isLoad = true;
    $scope.seveComboBtn = false;//套餐保存按钮
    $scope.seveItemBtn = false;//套餐保存按钮
    $scope.seveMaterialBtn = false;//套餐保存按钮
    if($stateParams.getWork.type == 0){
      $scope.title = '修改';
      HttpService.post({orderId: $stateParams.getWork.orderId}, customerHost+'api/newPlant/edit/editRepairOrder.api').then(function(result){
        ...
        if(result.status='0000'){
          ...
          $scope.orderMessage.mileage = parseFloat($scope.orderMessage.mileage);
          $scope.addItemRental(); //项目小计
          $scope.addMaterialRental();//材料小计

          // $scope.sendParam.orderList = result.data.imageList;//图片信息
    ...
    //全选取值
    var updateSelected = function (action, name, selectType) {
    ...
    //选择套餐,项目,材料
    $scope.showCombo = function(num,btn,seach, tabChangeName){
      if(tabChangeName){
        $scope.tabChangeName = tabChangeName;
      }
    ...
    //搜索清空后
    $scope.isSeachVal = function(val,num,btn){
      if(val==''){
        // alert(111)
    ...
    $scope.removeImg = function(_index, delNum){
      $scope.repairOrderStr.delNumber += delNum + ',';
    ...
    $scope.choosePicMenu = function (_index) {
      ...
      //$scope.imgList.otherImage.push('www.baidu.com');
      //$scope.imgList.number.push(num);
    };
