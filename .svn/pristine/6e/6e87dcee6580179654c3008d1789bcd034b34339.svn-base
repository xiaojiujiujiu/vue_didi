angular.module('starter.controllers', [])
  .controller('orderListCtrl', function($scope, orderListServices, $state,$ionicSlideBoxDelegate, $ionicTabsDelegate,HttpService,customerHost,Util,$stateParams) {
    ...
    $scope.loadMore = function (search) {
      if(!flag){
        flag = true;
        HttpService.post($scope.reviewList, customerHost+'api/newPlant/list/showRepairOrderList.api').then(function(result){
          // console.log(result)
          ...
          document.getElementById('itemsNum6').innerHTML = result.data.statusCount.check1Num;
          document.getElementById('itemsNum7').innerHTML = result.data.statusCount.check2Num;
