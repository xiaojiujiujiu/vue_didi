    ...
    return {
      fileUpload: function (image, upLoadImg, goodsType, scope, callBack) {
        ...
        var token = window.localStorage.getItem('loginInfo') ? JSON.parse(window.localStorage.getItem('loginInfo')).token : 'undefined';
          ...
          ft.upload(
            upLoadImg,
            encodeURI(customerHost + '/api/goods/uploadImage?vin=' + goodsType + '&token=' + token),//把图片及其他参数发送到这个URL，相当于一个请求，在后台接收图片及其他参数然后处理
            uploadSuccess,
            uploadError,
            options);
        ...
        function uploadSuccess(r) {
          ...
          if (resp.code == '200') {
            ...
            //$ionicHistory.back();
            if (typeof callBack == 'function') {
              //alert(resp.data.imageName+" "+resp.data.path);
              callBack(image, upLoadImg, resp.data.tempImagePath);
